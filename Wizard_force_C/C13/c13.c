...
void die(const char* message){
    if(errno){      // system error, use perror to print the msg
        perror(message);
    }
    else{
        printf("ERROR: %s\n", message);     // other errors, customed print warning
    }
    exit(1);
}
...
Connection* Database_open(const char* filename, char mode){
    ...
    if(!conn) die("ERROR MEMORY DISTRIBUTED");

    conn->db = (Database*)malloc(sizeof(Database));
    if(!conn->db) die("ERROR MEMORY DISTRIBUTED");
